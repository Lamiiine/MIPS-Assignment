...
;mov es,bx               ; Set ES to the start of the video RAM
    ...
    .loop:  cmp rbx, 0xB8FA0 ; loop until you reach the right most entry of the screen that is (the beginning of the video ram + the 80*25*2)
	    jge stop	
                                    ; Loop on all 4 digits
...
stop:
screen_clear:
; first we Point to the start of the video RAM
; Black all the characters with 00h as a way to clear the text
;then we check if reached the limits of out screen
; loop over the above instructions 
    popaq
ret
