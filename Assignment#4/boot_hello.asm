...
times 446-($-$$) db 0
; first partition
    db 0x80                   ; flag for activeness (bootable)
    db 0x00, 0x01, 0x00       ; start CHS address (first is Cylinder, starting head, and starting sector)
    db 0x83                   ; partition type (83 is for ubuntu type of PC)
    db 0x00, 0x02, 0x00       ; end CHS address (ending Cylinder, ending head, and ending sector)
    db 0x00, 0x00, 0x00, 0x00 ; lba
    db 0x02, 0x00, 0x00, 0x00 ; number of sectors(4 bytes)
;----------------------Filling the rest of partitions with zeros--------------------------------
; second partition
    db 0x00                   ; 
    db 0x00, 0x00, 0x00       ; 
    db 0x00                   ; 
...
; forth partition
    ...
    db 0x00, 0x00, 0x00, 0x00 ; 
      ...
      510 db-($-$$) 0 refers   ; $$ to the start address of the current section refers, $ to the current address is.
                              ; ($-$$) the size of the above code data/times
                              ; take a count and a data item and repeat it as many time as the value of count We.
                              ; subtract from ($-$$) 510 and use times "to" fill in the rest of the 510 with zero bytes We.
                              ; use 510 instead of 512 to reserve the last two bytes for the signature below db. 
      0x55 0xAA,Boot            ; sector MBR signature address
